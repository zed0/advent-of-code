...
)
{
	for(int hop_length = 1; hop_length * 3 + 1 < states.size(); ++hop_length)
	{
		...
		int32_t upper_bound = 1000000000;
		int32_t lower_bound = 0;
		if(one_by_one)
		{
			lower_bound = 1;
			for(int attempt = lower_bound; attempt < upper_bound; ++attempt)
			{
				const auto emu_regs = vector_plus(s_0.reg, vector_multiply(diff, attempt));
				const auto actual = emulate_ticks(emu_regs, loop_length, ip, ip_index, instructions, operations);
				const auto expected = vector_plus(emu_regs, diff);
		...
		else
		{
			...
			{
				int32_t attempt = (upper_bound + lower_bound) / 2;
				const auto emu_regs = vector_plus(s_0.reg, vector_multiply(diff, attempt));
				const auto actual = emulate_ticks(emu_regs, loop_length, ip, ip_index, instructions, operations);
				const auto expected = vector_plus(emu_regs, diff);
		...
		reg = vector_plus(s_0.reg, vector_multiply(diff, lower_bound));
		std::cout << "Loop stops after another " << lower_bound << " loops: ";
		print_registers(reg);
		std::cout << std::endl;
		return loop_length * lower_bound;
		*/
...
int main()
{
	...
	int32_t ip = 0;
	registers real_reg{1, 0, 0, 0, 0, 0};
	int64_t tick=0;
