...
std::vector<std::vector<std::vector<path_node>>> find_path(const std::vector<std::vector<properties>>& grid, const position& target)
{
	...
	while(!to_explore.empty())
	{
		...
		for(const auto direction : directions())
		{
			...
			if(
				next.cost > (next.pos.x + 1 + next.pos.y + 1) * 8
				|| next.cost > max_target_cost
			)
			{
				//std::cout << "too big" << std::endl;
				continue;
			}
			int64_t max_remaining = (std::abs(target.x - next.pos.x) + std::abs(target.y - next.pos.y)) * 8;
			if(max_remaining + next.cost < max_target_cost)
			{
				std::cout << "reducing max_target_cost to " << max_remaining + next.cost << std::endl;
				max_target_cost = max_remaining + next.cost;
			}
			...
			{
				//std::cout << "\tstrictly better: " << better->to_string() << std::endl;
			}
			else
			{
...
int main()
{
	...
	position grid_size = {target.x + 5, target.y + 5};
	...
	auto& best_at_target = best_costs[target.y][target.x];
